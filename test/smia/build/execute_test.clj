(ns smia.build.execute-test
  (:require
   [smia.build.execute :as execute]
   [smia.error :as error]
...
(defn- tmp-dir [tag]
  (str (System/getProperty "java.io.tmpdir")
       "/smia-execute-" tag "-" (System/currentTimeMillis)))
...
(deftest print-x-without-config-fails-fast-at-prepare
  (testing "the gate trips before any loading or rendering"
    (let [d (catch-data #(execute/prepare (request "px" :editions [:print-x])))]
      (is (= :smia.build.request/print-x-requires-config (:error/type d)))))
