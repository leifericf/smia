...
- `:book/downloads` — a base URL and a list of downloadable editions; the `site` edition turns it into a Downloads page. A map with a string `:base` and an `:assets` vector of `{:label :file :note? :default?}` maps, with at most one asset marked `:default`. See [the distribution chapter](#distribution).
