...
- `:book/identifier`, `:book/language`, `:book/accessibility`: EPUB package metadata, all optional with defaults. `:book/language` does double duty: it also localizes the apparatus smia generates — the float and structure labels ("Figure", "Chapter"), the generated section titles ("Bibliography", "List of Figures"), the admonition labels, and the site chrome — in every edition. English is built in; an unknown language falls back to it. The manuscript's own content is never translated, only the furniture around it.
