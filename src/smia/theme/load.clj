...
(def required-groups
  "Required top-level token groups. `smia.schema/Tokens` is the schema
...
(defn validate
  ...
  [tokens _path]
  (schema/check schema/Tokens tokens :smia.theme.load/invalid-tokens))
...
(defn load-tokens
  ...
  (let [f (io/file book-root "theme.edn")]
    (when-not (.exists f)
      (throw (error/ex :smia.theme.load/missing
