(ns smia.schema
  "Malli schemas for the values that flow between smia's contexts.
...
(def Request
  "A normalized public request map (see `smia.build.request`)."
  [:map
...
(def Plan
  "An inspectable, pure description of a build: which editions, where each
   ...
   smia.build.plan; performed by smia.build.execute."
  [:map
   [:book-root :string]
