(ns clj-book.html.expand
  "Pure expansion of the author Hiccup vocabulary into HTML-Hiccup.

   The sibling of `clj-book.fo.expand`: the same three concentric layers,
   ...
   3. Book extensions expand to classed HTML idioms; styling lives in
      CSS (see `clj-book.theme.css`), never inline.

   The `ctx` carries `:resolve` — `(fn [id] href)`, built by the
   assembler from `clj-book.html.links` — and `:highlight?`. Footnotes
     ...
     (cond
       ...
       (let [[tag attrs children] (hiccup/parse-node node)]
         (cond
           ...
           (throw (error/ex :clj-book.html.expand/fo-tag-in-html
           ...
           :else
           (throw (error/ex :clj-book.html.expand/unknown-tag
       ...
       :else
       (throw (error/ex :clj-book.html.expand/invalid-node
  ...
  (reduce
    (fn [acc [i {:keys [line note]}]]
      (when-not (and (integer? line) (<= 1 line line-count))
        (throw (error/ex :clj-book.html.expand/invalid-annotation
...
(defn- footnote-ref
  ...
  (let [n (:n author)]
    (when-not n
      (throw (error/ex :clj-book.html.expand/unnumbered-footnote
...
(defn- xref [author children ctx]
  (let [dest (as-id (:to author))]
    (when-not dest
      (throw (error/ex :clj-book.html.expand/invalid-xref
...
(defn- cite [author ctx]
  (let [key (:key author)]
    (when-not key
      (throw (error/ex :clj-book.html.expand/invalid-cite
...
(def expanders
  ...
  {:p          (passthrough :p)
   ...
   :img        (fn [a _ _]
                 (when-not (:alt a)
                   (throw (error/ex :clj-book.html.expand/missing-alt-text
