...
(defn- resolve-evaluator
  ...
  [lang]
  (when (nil? lang)
    (throw (error/ex :clj-book.eval/missing-language
  ...
  (let [{:keys [evaluate requires] :as desc} (get registry/evaluators lang)]
    (when-not desc
      (throw (error/ex :clj-book.eval/unsupported-language
    ...
    (or (try (requiring-resolve evaluate)
             (catch Exception e
               (throw (error/ex :clj-book.eval/evaluator-unavailable
...
(defn validate-blocks!
  ...
  (let [results  (mapv (fn [{:keys [lang] :as block}]
    ...
    (when (seq failures)
      (throw (error/ex :clj-book.eval/validation-failed
